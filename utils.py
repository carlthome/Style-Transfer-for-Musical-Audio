...
def get_stft_kernels(n_dft):
    """This is the tensorflow version of a function created by Keunwoo Choi
    shown here: https://github.com/keunwoochoi/kapre/blob/master/kapre/stft.py.
...
def atan2(y, x, epsilon=1.0e-12):
    ...
    angle = tf.where(tf.logical_and(tf.less(x, 0.0),  tf.greater_equal(
        y, 0.0)), tf.atan(y / x) + np.pi, angle)
    angle = tf.where(tf.logical_and(tf.less(x, 0.0),  tf.less(
        y, 0.0)), tf.atan(y / x) - np.pi, angle)
    angle = tf.where(tf.logical_and(tf.equal(x, 0.0), tf.greater(
        y, 0.0)), 0.5 * np.pi * tf.ones_like(x), angle)
...
def conv1d(x,
           ...
           kernel=None):
    """Taken from Magenta's NSynth Wavenet Model Fast 1D convolution that
    supports causal padding and dilation.

    Args: x: The [mb, time, channels] float tensor that we convolve.
    num_filters: The number of filter maps in the convolution.
    filter_length: The integer length of the filter. name: The name of
    the scope for the variables. dilation: The amount of dilation.
    causal: Whether or not this is a causal convolution.
    kernel_initializer: The kernel initialization function.
    biases_initializer: The biases initialization function. Returns: y:
    ...
    if kernel == None:
        std = np.sqrt(2) * np.sqrt(2.0 /
                                   ((num_input_channels + num_filters) * filter_length))
...
def _mel_frequencies(n_mels=128, fmin=0.0, fmax=11025.0):
    ...
    `htk` is removed. copied from Librosa

    """
    def _mel_to_hz(mels):
        """Convert mel bin numbers to frequencies copied from Librosa."""
    ...
    def _hz_to_mel(frequencies):
        """Convert Hz to Mels copied from Librosa."""
...
def _dft_frequencies(sr=22050, n_dft=2048):
    """Alternative implementation of `np.fft.fftfreqs` (said Librosa) copied
    from Librosa."""
...
def mel(sr, n_dft, n_mels=128, fmin=0.0, fmax=None):
    ...
    use Slaney copied from Librosa, librosa.filters.mel.
...
def get_variables(y, ndft_else,
    ...
    fft_basis_tf = tf.constant(
        fft_basis, name="fft_basis_" + cqt_name, dtype='float32')
    ...
    if n_fft == ndft_else:
        ...
        dft_real_kernels_cqt_tf = tf.constant(
            dft_real_kernels_cqt, name="dft_real_kernels_cqt_" + cqt_name, dtype='float32')
        dft_imag_kernels_cqt_tf = tf.constant(
            dft_imag_kernels_cqt, name="dft_imag_kernels_cqt_" + cqt_name, dtype='float32')
    ...
    if not scale:
        ...
        lengths_tf = tf.constant(
            lengths, name="lengths_" + cqt_name, dtype='float32')
    else:
        lengths_tf = None
...
def get_pseudo_cqt(x_,
    ...
    if dft_kernels != None:
        ...
        _, STFT_magnitude, _ = get_logmagnitude_STFT(
            x_, dft_real_kernels_cqt_tf, dft_imag_kernels_cqt_tf, n_hop)
